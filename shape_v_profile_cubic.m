...
%       這個系統裡 a1 固定是 0（見下方「邊界條件」說明），程式碼直接把
%       a1=0 代入上面兩條公式化簡（-a1/s1、+a1/s1^2 這兩項消失），不在
%       程式裡另外留一個永遠是 0 的變數。
%
% 邊界條件在這個系統裡怎麼決定：
...
function v_profile_shaped = shape_v_profile_cubic(refpath_active, v_now, a_now, params)
    ...
    v0 = v_now;
    a0 = a_now;
    % a1（終點加速度邊界條件）固定是 0——假設到達最緊的點時已經穩定、
    % 不再加減速（見檔頭「邊界條件」說明）。已經代入化簡，公式裡因此
    % 沒有 -a1/s1、+a1/s1^2 這兩項。

    q1 = v0;
    q2 = a0;
    q3 = -2*a0/s1 - 3*v0/s1^2 + 3*v1/s1^2;
    q4 =  a0/s1^2 + 2*v0/s1^3 - 2*v1/s1^3;
