...
% =========================================================================
% 這一節是「決策分支路徑視覺化」新功能的參數：把候選路徑從「N 條連續
% 平行偏移」改成「離散車道狀態機」——固定只有左/中/右 3 條車道，在中間
% 車道時有 3 種選擇（直行、變左、變右），在左右車道時只有 2 種選擇
% （直行、變回中間），每種選擇畫成一條分支曲線，末端用 X 標記代表
% 「下一個決策點」。詳細設計見 generate_decision_branches.m 檔頭。
params.decision_lookahead_time_s = 5.0;  % [s] 決策視野時間：決策點間距
                                          %     decision_spacing_m =
                                          %     min(v*decision_lookahead_time_s,
                                          %     decision_spacing_max_m)，
                                          %     跟速度成比例（模擬真實駕駛
                                          %     「開越快看越遠再想下一步」），
                                          %     在 v_des=6m/s 時剛好等於
                                          %     decision_spacing_max_m=30m。
params.decision_spacing_max_m    = 30;   % [m] 決策點間距上限（X 標記最遠
                                          %     距離），不管車速多快都不會
                                          %     超過這個值。
params.lane_change_time_s        = 4.0;  % [s] 完成一次換道所需時間，
                                          %     lane_change_distance =
                                          %     v * lane_change_time_s，
                                          %     一樣跟速度成比例（像真實
                                          %     駕駛：車速快，換道實際掃過
                                          %     的距離就比較長）。3~5 秒是
                                          %     常見的真實單次換道時間範圍，
                                          %     取中間值 4 秒。
params.lane_change_wp_spacing    = 0.5;  % [m] 換道斜坡區段（側向偏移量
                                          %     快速變化的那一段）的 waypoint
                                          %     抽稀間距，比一般候選路徑用的
                                          %     local_wp_spacing(1.5m) 更密，
                                          %     因為換道瞬間的航向變化率遠
                                          %     高於單純沿車道直行，取樣太粗
                                          %     會讓擬合出來的換道曲線在
                                          %     waypoint 交界處看起來有稜角
                                          %     （原理見 generate_decision_
                                          %     branches.m 檔頭「換道曲線的
                                          %     航向修正」說明）。
params.lane_change_min_distance_m = 8;   % [m] 換道距離下限，避免車速極低
                                          %     時 v*lane_change_time_s 趨近
                                          %     0，導致換道斜坡的弧長參數化
                                          %     除以接近 0 的距離而數值不穩定。
